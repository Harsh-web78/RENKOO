...
import { filterHistoryByProperty, resolveHistoryView } from "@/lib/data-contract/real-state";
...
/**
 * Real-mode History: the list comes from the backend (workspace history
 * filtered to the active property). Never session-local mock data.
 */
function RealHistoryContent() {
  const session = useSession();
  const propertyId = session.onboarding.propertyId;

  const items = propertyId ? filterHistoryByProperty(session.realHistory, propertyId) : [];
  const view = resolveHistoryView(
    !session.realSessionLoaded ? "loading" : session.realHistoryStatus,
    session.realHistoryError,
    items,
  );
  const currentFix = propertyId ? session.getPropertyState(propertyId).currentFix : null;
  ...
  return (
    <div className="py-10 md:py-16">
      <Container width="page">
        ...
        {view.kind === "loading" && (
          <div className="mt-6">
            <RealLoadingSkeleton label="Loading history…" />
          </div>
        )}
        {view.kind === "error" && view.error && (
          <div className="mt-6">
            <RealErrorPanel error={view.error} onRetry={() => session.reloadRealHistory()} />
          </div>
        )}
        {view.kind === "empty" && (
          <div className="mt-6 rounded-sm border border-line p-6">
            ...
            <Button variant="primary" type="button" className="mt-4" href={currentFix ? "/fixes" : "/home"}>
              {currentFix ? "View current fix" : "Return to Home"}
            </Button>
          </div>
        )}
        {view.kind === "list" && (
